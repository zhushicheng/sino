<template>
  ...
  <div class="usermanage batch-empass">
    <sino-con class="empass-sino-icon">
      <sino-title style="margin-bottom: 15px" type="levelOne" title="修改订单开始日期"></sino-title>
        ...
        <div class="empass-middle-left fl">
          ...
          <ul class="left-bottom">
            ...
            <li>
              ...
              <div>
                <p>失败数据</p>
                <p><span class="tip_num" v-text="batchAddform.failNum || 0"></span><span v-if="batchAddform.failNum" style="cursor: pointer" @click="batchFailDownload(batchAddform.batchId)">下载</span></p>
              </div>
            </li>
          </ul>
        </div>
      ...
      <sino-tabs v-model="activeName" type="card" @tab-click="handleClick" class="sino-tab-decl sino-tab-decl-empass">
        <sino-tab-pane label="成功数据" name="success" class="mt15">
          <sino-table :data="successTableData" max-height="416" stripe border v-on:viewAll="successViewAll">
            ...
            <sino-table-column prop="begDate" label="订单开始日期" min-width="100"></sino-table-column>
            <sino-table-column prop="endDate" label="订单结束日期" min-width="100"></sino-table-column>
            ...
            <sino-table-column prop="paySuppId" label="付费供应商简称" min-width="100"></sino-table-column>
          </sino-table>
...
//      this.drawPie();
    ...
    methods: {
      ...
      successViewAll (pageNum, pageSize) {
        ...
        api.post(this.apiPath.hrsc.pageOrderBatchModifyBegDateSucc, _data).then(res => {
      ...
      errorViewAll (pageNum, pageSize) {
        ...
        api.post(this.apiPath.hrsc.pageOrderBatchModifyBegDateFail, _data).then(res => {
      ...
      batchFailDownload (batchNo) {
        api.postsign(this.apiPath.hrsc.downloadOrderBatchModifyBegDateFailData, batchNo).then(res => {
          if (res.code === 'CPYY-00001') {
            api.download(res.data);
          }
        }).catch(err => {
      ...
      batchInport () {
        this.$refs.batchLeadForm.validate((valid) => {
          if (valid) {
            ...
            api.postsign(this.apiPath.hrsc.impOrderUpBegDateExcel, this.batchLeadForm.fileUniqueNo).then(res => {
              if (res.code === 'CPYY-00001') {
                this.batchLeadFlag = false;
      ...
      // 模板下载
      downloadFun () {
        api.postsign(this.apiPath.hrsc.downloadByTypeByorder, 38).then(res => {
          if (res.code === 'CPYY-00001') {
            api.download(res.data);
          }
        }).catch(err => {
