<template>
  ...
  <div>
    <div class="addbuscon" v-if="!routerView">
      <sino-con>
        <sino-title type="levelOne" title="新增商务合同"></sino-title>
        <sino-form :model="addBusConForm" :rules="addBusConRules" ref="addBusConForm" class="addbuscon-search label-width-81">
          ...
          <sino-form-item label="客户名称：" class="sino-form-item-width"  prop="custChName" v-if="conSignObjFlag">
            <sino-select :filterable="true"
                         ...
                         :clear="true"
                         @contentChange="getCertNo(addBusConForm.custChType)"
                         class="fl">
            </sino-select>
          ...
          <sino-form-item label="供应商名称：" class="sino-form-item-width"  prop="chName" v-if="!conSignObjFlag">
            <sino-select :filterable="true"
                         ...
                         :clear="true" class="fl"></sino-select><!-- @contentChange="chName"-->
          </sino-form-item>
          <sino-form-item label="关联销售线索：" class="sino-form-item-width"  prop="outline">
            <sino-select :filterable="true"
                         :multiselect="false"
                         :isshowTotol="false"
          ...
          <sino-form-item label="合同名称：" prop="conName" class="addbuswid">
            <sino-input v-model="addBusConForm.conName" ></sino-input>
          </sino-form-item>
          <sino-form-item label="合同编号：" prop="contNo">
            <sino-input v-model="addBusConForm.contNo" ></sino-input>
          </sino-form-item>
          ...
          <!--</sino-form-item>-->
          <sino-form-item label="合同联系人：" prop="contName" class="label-width-156">
            <sino-select :filterable="false"
                         :multiselect="false"
                         :isshowTotol="false"
                         :disabled="false"
          ...
          <sino-form-item label="签约日期：" prop="signDate" class="label-width-156">
            <sino-date-picker
          ...
          <sino-form-item label="顺延月数：" prop="postponeMonth" v-if="addBusConForm.isPostpone !== 0">
            <sino-input v-model="addBusConForm.postponeMonth"></sino-input>
          </sino-form-item>
          ...
          <sino-form-item label="合同总份数：" prop="conTotalAmo" class="clear">
          ...
          <sino-form-item label="约定付款日：" prop="payDate" >
            ...
            <sino-input v-model="addBusConForm.payDate"></sino-input>
          </sino-form-item>
          <sino-form-item label="次月付款：" prop="payNextMonth" class="radio-item">
            <sino-radio-group v-model.number="addBusConForm.payNextMonth">
              <sino-radio v-for="(item, index) in isPostponeList" :key="index"
                          :label="item.id"
              >{{item.title}}</sino-radio>
            </sino-radio-group>
    ...
    data () {
      ...
      return {
        ...
        addBusConRules: {
          ...
          outline: [
            {required: true, message: '请输入关联销售线索', trigger: 'change'}
          ],
          certNo: [
...
//          ],
          ...
          postponeMonth: [
            {required: true, message: '请选择顺延月数', trigger: 'change'},
          ...
          payDate: [
            {required: true, message: '请输入约定付款日', trigger: 'change'},
    ...
    watch: {
      ...
      $route: {
        handler: function (val) {
          if (val.path === "/custBusCon/addBusCon") {
    ...
    mounted () {
      ...
      if (this.$route.query.saleClueId !== undefined) {
        ...
        if (this.$route.query.custChType === undefined && this.$route.query.custChName === undefined) {
    ...
    methods: {
       ...
       */
      getCertNo (id) {
        if (this.addBusConForm.conSignObj === 3) {
          this.addBusConForm.certNo = '';
          api.post(this.apiPath.hrsc.findCertNoById, {id: id}).then(res => {
            if (res.code === 'CPYY-00001') {
              this.addBusConForm.certNo = res.data.certNo;
              console.log(this.addBusConForm.certNo, 'this.addBusConForm.certNo');
            }
          }).catch(err => {
            console.log(err);
          });
...
//          custOrSuppId: val.id
//        };
//        api.post(this.apiPath.hrsc.judgeSuppExistCon, obj).then(res => {
//          if (res.code === 'CPYY-04415') {
...
//      },
      ruoteJudge () {
        if (this.$route.path !== "/custBusCon/addBusCon") {
...
//            if (_this.addBusConForm.conSignObj === 1) { // 供应商判断
//              let obj = {
//                conSignObj: 1,
//                custOrSuppId: _this.addBusConForm.suppId,
//                begDate: _this.addBusConForm.begDate
//              };
//              api.post(_this.apiPath.hrsc.judgeSuppExistCon, obj).then(res => {
//                if (res.code === 'CPYY-00001') {
//                  _this.saveFun();
//                } else {
...
//            }
      ...
      saveFun () {
        ...
        api.post(_this.apiPath.hrsc.findExitsConInfo, _this.addBusConForm).then(res => {
          if (res.code === 'CPYY-00001') {
            if (res.data.dateRepeat) {
              this.$alert('该供应商/客户同一时间已经签署过合同，确认继续添加?', '温馨提示', {
                confirmButtonText: '确 定',
                type: 'warning', // icon图标类型
                dragFlag: true,
                customClass: '', // 添加class
                lockScroll: true,
                callback: action => {
                  api.post(_this.apiPath.hrsc.save, _this.addBusConForm).then(res => {
                    if (res.code === 'CPYY-00001') {
