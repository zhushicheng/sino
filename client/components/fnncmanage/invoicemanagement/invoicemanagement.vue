<template>
  <div class="usermanage">
        ...
        <sino-form :model="searchForm" ref="searchForm" class="serch-form">
          <div class="search-box">
            <ul class="search-inner-box invoice-ul">
              ...
              <li>
                <span class="search-inner-name">费用生成年月：</span>
                <sino-date-picker
              ...
              <li>
                ...
                <sino-select class="fl"
                             ...
                             :isPage="false"
                             :isGroup="false"
                             :data="{code: 'CHARGE_STATE',ids: [1,2,3]}"
                             :url="getDictSelectBox"
        ...
        <div class="toll-bill-ul-box">
          <ul class="toll-bill-table-ul clearfix invoice-checked-box">
            ...
            <li class="invoice-fr-li">
              <h6>总金额:</h6>
              <i>&yen;</i>
              <span class="mr10">{{totalAmount}}</span>
            </li>
          </ul>
        </div>
    ...
    <!--开票弹层-->
    <sino-dialog :visible.sync="billingAdd" size="samll" dialogHide @close="billingAddClose('billingForm')" class="usermanage" top="5%">
      ...
      <div class="usermanagement-modify1">
        <sino-form :model="billingForm" :rules="billingRules" ref="billingForm" class="fl">
          <sino-form-item label="票据类型：" prop="billType" class="billing-form-smallbox">
          <sino-radio v-for="(item, index) in billingList" :key="index" v-model.number="billingForm.billType" :label="item.id">{{item.title}}</sino-radio>
          </sino-form-item>
          <div v-if="!hideShow">
          <sino-auto-table :data="billingData" stripe style="width:744px"  max-height="236" border>
          <sino-table-column type="index" label="序号" align="center" width="50"></sino-table-column>
          <sino-table-column prop="itemName" label="开票项目" align="center" min-width="161" ></sino-table-column>
          ...
          <sino-table-column prop="systemSummerInvSub" label="发票科目" align="center" min-width="161" :editType='true'>
            <template scope="scope">
            <select v-show="scope.row.editSelect" v-model="scope.row.invSub"
            @change="changconetent(scope.row, options)">
          ...
          <option v-for="(option, index) in invoiceOptions" :key="index" :label="option.title" :value="option.title">
          {{ option.title }}
          </option>
          </select>
          ...
          <template scope="scope">
            <select v-show="scope.row.editSelect3" v-model="scope.row.drawerMan"
                    @change="scope.row.editSelect3 = false;">
              <option v-for="(option, index) in drawer" :key="index" v-bind:value="option.title">
                {{ option.title }}
              </option>
            </select>
          <span v-show="!scope.row.editSelect3" @click="scope.row.editSelect3 = true;" class="defaultVa">{{ scope.row.drawerMan }}</span>
          </template>
          </sino-table-column>
          <sino-table-column prop="drawerTime" label="开票日期" align="center" min-width="161" :editType='true'>
          <template scope="scope">
          <sino-date-picker v-show="scope.row.editDate" v-model="scope.row.drawerTime" type="date" :editable="false" :readonly="false" format="yyyy-MM-dd"
      ...
      <div class="usermanagement-modify1">
        <sino-form :model="receiveForm" :rules="receiveRules" ref="receiveForm" class="fl">
          <sino-form-item label="领取方式：" prop="id" class="billing-form-smallbox">
            <sino-radio v-for="(item, index) in receiveList" :key="index"
                        :label="item.id"
        ...
        <sino-form-item prop="fileName" label="上传文件：" class="sino-form-item-width is-required">
          ...
          >
            <div slot="trigger">
              ...
              <input class="uploadFileBox" type="text" v-model="fileName" readonly="true"/> <!--v-model="modifyAndAddForm.busLicName" -->
    ...
    <sino-dialog :visible.sync="invoiceFlag" size="small" @close ="invoiceContent('transForm')">
      ...
      <div style="height: 300px; overflow: auto">
        <div v-for="(item, index) in remiSeeList" :key="index"  ><!--v-if="choosedId.indexOf(item.invInfoId) === -1"-->
          <sino-button type="text" class="fr" v-if="item.isShow" @click="cancelTicket(item.invInfoId)">取消开票</sino-button>
          <div class="usermanagement-list marBo">
            <div v-if="item.billTypeName !== '收据'">
              <ul class="usermanagement-ul" v-if="item.cancelInvMan && item.cancelInvTime && item.cancelInvReason">
                <li class="li-last">
                ...
                <li>
                  <h6>开票日期</h6><span v-text="item.drawerTime"></span>
                </li>
                <li>
                  <h6>取消人</h6><span v-text="item.cancelInvMan"></span>
                </li>
                <li>
                  <h6>取消日期</h6><span v-text="item.cancelInvTime"></span>
                </li>
                <li>
                  <h6>取消原因</h6><span v-text="item.cancelInvReason"></span>
                </li>
              </ul>
              <ul class="usermanagement-ul" v-else>
                <li class="li-last">
                  <h6>票据类型</h6><span v-text="item.billTypeName"></span>
                </li>
                <li class="li-last">
            ...
            <div v-if="item.billTypeName === '收据'">
              <ul class="usermanagement-ul">
                ...
                <li>
                  <h6>开票日期</h6><span v-text="item.drawerTime"  class="borderRig"></span>
                </li>
              </ul>
            </div>
          </div>
    ...
    <!--取消信息弹层-->
    <sino-dialog :visible.sync="cancelBillAdd" size="samll"  @close="dialogClose('cancelBillForm')" class="usermanage dialog-auto">
  ...
  export default {
    ...
    data () {
      return {
        ...
        receipt: {
          invAmount: null,
          drawerMan: Cookie.get('CPYY-INFO-NAME'),
          drawerTime: '',
          billId: '',
          billNo: '',
          billType: null,
...
//        show: false,
        ...
        billingRules: {
          billType: [
            {required: true, message: '请选择票据类型', trigger: 'change', type: 'number'}
          ],
          custChName: [
      ...
      billingForm: {
        handler: function (billingForm) {
          ...
          this.billingData.forEach(function (item, index) {
            ...
            item.drawerMan = Cookie.get('CPYY-INFO-NAME');  // 开票人-----
    ...
    methods: {
      ...
      checkFn () {},
      resetFun () {
        for (let key in this.billSuppCustFrom) {
          this.billSuppCustFrom[key] = null;
        }
      },
      socSec () {
      ...
      // 获取发票科目
      obtainingInvoice () {
        api.post(this.getDictSelectBox, {code: 'SYSTEM_SUMMER_INV_EMU'}).then(res => {
      ...
      huoquData () {
        api.post(this.getDictSelectBox, {code: 'BILL_TYPE', ids: [1, 3]}).then(res => {
      ...
      // 获取发票类型数据
      invoiceTypeData () {
        api.post(this.getDictSelectBox, {code: 'INV_TYPE'}).then(res => {
      ...
      drawerData () {
        api.post(this.apiPath.author.findUserByOrgIdsAndRoleType, {code: 'ROLE_TYPE', roleTypes: [18, 19, 20]}).then(res => {
          this.drawer = res.data.list;
        }).catch(err => {
          console.log(err);
        });
      },
...
//          this.$refs.fileNameFile.submit();
      ...
      cancelBillSubmitFun () {
        let _this = this;
        _this.$refs.cancelBillForm.validate((valid) => {
          if (valid) {
            ...
            api.post(this.apiPath.hrsc.updateCancelDrawerList, checkedArr).then(res => {
              if (res.code === 'CPYY-00001') {
                ...
                _this.cancelBillForm.cancelInvTime = new Date(_this.cancelBillForm.cancelInvTime);
              }
            }).catch(err => {
              console.log(err);
            });
      ...
      billingFun (item) {
        ...
        //     billingDeterData1.chargeBillId = data.chargeBillId;
        //     billingDeterData1.type = data.type;
        //     billingDeterData1.chargeBillNo = data.chargeBillNo;
        ...
        //     billingDeterData2.chargeBillId = data.chargeBillId;
        //     billingDeterData2.type = data.type;
        //     billingDeterData2.chargeBillNo = data.chargeBillNo;
        ...
        //     billingDeterData3.chargeBillId = data.chargeBillId;
        //     billingDeterData3.type = data.type;
        //     billingDeterData3.chargeBillNo = data.chargeBillNo;
        ...
        //     billingDeterData4.chargeBillId = data.chargeBillId;
        //     billingDeterData4.type = data.type;
        //     billingDeterData4.chargeBillNo = data.chargeBillNo;
        ...
        let obj = {
          ...
          type: item.type,
          billNo: item.chargeBillNo
        };
        api.post(this.apiPath.hrsc.showSaveInvInfo, obj).then(res => {
          if (res.code === 'CPYY-00001') {
            if (res.data.saveInvInfoDTOS.length) {
              this.billingAdd = true;
              res.data.saveInvInfoDTOS.forEach(function (item, index) {
                item['edit'] = false;
                item['editDate'] = false;
                item['editSelect'] = false;
                item['editSelect2'] = false;
                item['editSelect3'] = false;
                // item['systemSummerInvSubName'] = '';
                item['drawerMan'] = Cookie.get('CPYY-INFO-NAME');
              }, this);
              var that = this;
              setTimeout(function () {
                that.billingData =  res.data.saveInvInfoDTOS;
              }, 500);
              this.billingForm.chargeBillId = res.data.chargeBillId;
              this.billingForm.billNo = res.data.billNo;
              this.billingForm.type = res.data.type;
              return;
            }
          }
        }).catch(err => {
      ...
      billingSubmitFun () {
        this.$refs.billingForm.validate((valid) => {
          ...
          if (valid) {
            this.billingData.forEach(function (item, index) {
              if (item['invTypeName'] == '普票') {
                item['invType'] = this.invoiceOptions[0].id;
              }
              ;
              if (item['invTypeName'] == '专票') {
                item['invType'] = this.invoiceOptions[1].id;
              }
              ;
            }, this);
            this.billingData.forEach(item => {
              item.drawerTime = Vue.filter('date')(item.drawerTime, 'yyyy-MM-dd');
            });
            let obj = {
              saveInvInfoDTOS: this.billingData,
              billNo: this.billingForm.billNo,
              type: this.billingForm.type,
              billType: this.billingForm.billType,
              chargeBillId: this.billingForm.chargeBillId
            };
            api.post(this.saveInvInfo, obj).then(res => {
              if (res.code === 'CPYY-00001') {
                this.billingAdd = false;
                checkedArr = [];
                this.searchFun();
      ...
      changconetent (item, arr) {
        arr.forEach(self => {
          if (self.id == item.invSub) {
            item.systemSummerInvSub = self.title;
          };
        });
        item.editSelect = false;
      },
      ...
      getList (all) {
        ...
        for (let item of this.saleData) {
          if (this.billSuppCustFrom.busiCustId && item.id === this.billSuppCustFrom.busiCustId) {
