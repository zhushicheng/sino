      ...
      <sino-con class="first-con" :class="{'fixed-con':scrollDist > 37}" id="first-con-id">
        ...
        <div class="crumbs style-crumb">
          <ul class="sixflex width-log">
            ...
            <li :class="{'li-active': activeNode === 6}"><span @click="stepFun(6)"><i class="step-num iconfont">6</i>领取鉴定结果</span></li>
            <li :class="{'li-active': activeNode === 7}"><span @click="stepFun(7)"><i class="step-num iconfont">7</i>已发放鉴定书</span></li>
          </ul>
        </div>
      </sino-con>
      <sino-con>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 返回
      retur () {
        this.$router.push({path: '/apprLaboCapaHandle'});
      },
      // 下载
      batchAllDownload (id) {
        api.post(this.apiPath.hrsc.downloadFilePublic, {uniqNo: id}).then(res => {
          ...
          if (res.code === 'CPYY-00001') {
            ...
            if (res.data.injApprMats.length !== 0) {
              ...
              res.data.injApprMats.forEach((item, index) => {
                ...
                if (_this.smallArr.length === 2 || index === res.data.injApprMats.length - 1) {
