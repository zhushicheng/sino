      ...
      <div class=" sino-table-total mt15">
        <div class="sino-table-icon bs-none">
          <div class="sino-table-icon-left">
            <sino-button type="icon-text" @click="add()"><i class="iconfont sino-Group"></i>合并
            </sino-button>
          </div>
        </div>
        <sino-table  :data="tableData" stripe max-height="416" v-on:viewAll="viewAll" @select-all='selectTabelRow' border @select="selectCallBack">
          ...
          <sino-table-column prop="tempName" label="账单模板名称"></sino-table-column>
          ...
          <sino-table-column prop="chargeBillTempProdTypeName" label="涉及产品类型">
...
<script>
  ...
  export default {
    ...
    created () {
      this.searchForm.chargeBillId = this.$route.query.chargeBillId;
      this.searchForm.suppId = this.$route.query.suppId;
      this.searchFun();
    },
    mounted () {
    ...
    methods: {
      ...
      getList (all) {
        ...
        api.post(this.apiPath.hrsc.findChargeBillMerge, obj).then(res => {
          res.data.list.forEach(function (item) {
            item.chargeBillTemp.prodTypes.forEach(function (item2) {
              console.log(item2.chargeBillTempProdTypeName);
            });
          });
...
//        改变分页显示条数;
      ...
      add: function (item) {
        let obj = {
          chargeBillId: this.searchForm.chargeBillId,
          mergerChargeBillIds: []
        };
        checkedArr.forEach(function (item) {
          obj.mergerChargeBillIds.push(item.chargeBillId);
        });
        api.post(this.apiPath.hrsc.updateChargeBillMerge, obj).then(res => {
          this.$message({message: '合并成功！', type: 'success'});
          this.routerView = true;
          this.$router.push({path: '/entrustedcharge', query: {suppId: this.searchForm.suppId}});
        }).catch(err => {
          console.log(err);
        });
      },
