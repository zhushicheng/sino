<template>
  <div class="roleMange">
    <div class="role-content">
      ...
      <div style="background: #fff;margin-top: 10px">
        <div class="sino-table-icon-left configuration-table-title">
          <sino-button v-if='false' type="text" @click="clickFun('','add')" :style="{'margin-left':'21px','padding-top':'5px','width':'150px'}"><i class="iconfont sino-zhangdan-copy"></i>应用到其它模版
          </sino-button>
        </div>
        <div class="etyma-content small-etyma-content fl configuration-table" :style="{'margin-right':'3%','padding-left':'20px'}">
          <div>
            ...
            <div class="sino-table-total configuration-table-box">
              ...
              <div class="configuration-table-box-childa"><span>已选<span :style="{'color':'#399ff2'}">{{checkLenth}}/{{total}}</span>项</span></div>
              <sino-table :data="tableDataA" max-height="416" class="get-check-table"  v-on:select="selectCallBack" @select-all="selectCallbackAll" border stripe>
        ...
        <div class="etyma-content small-etyma-content fl configuration-table">
            <div class="etyma-content-box" >
              ...
              <div class="sino-table-total configuration-table-box" >
                ...
                <div class="configuration-table-box-childa">已选<span :style="{'color':'#399ff2'}">{{checkLenth2}}/{{total2}}</span>项</div>
                <sino-table :data="tableDataB" v-on:select="selectCallBack2" @select-all="selectCallback2All" stripe border max-height="416" :row-style="showTr">
                  <sino-table-column  type="selection"></sino-table-column>
                  <sino-table-column  type="index" label="序号"></sino-table-column>
                  <sino-table-column prop="chName" label="中文名称"></sino-table-column>
                  <sino-table-column prop="enName" label="西文名称"></sino-table-column>
                  <sino-table-column prop="showSort" label="排序" :editType='true' width="60" align="center">
                    <template scope="scope">
                      <!-- <sino-input v-if="scope.row.edit" @blur="scope.row.edit = false"
                                  v-model="scope.row.showSort" ></sino-input> -->
                      <!--<span v-if="!scope.row.edit"  @click="scope.row.edit = true;change(scope.row.showSort, scope.row.edit)" >{{ scope.row.showSort}}</span>-->
                      <sino-input v-show="scope.row.edit"
                                  v-model="scope.row.showSort" @blur="scope.row.edit = false;change(scope.row)"></sino-input>
                      <span v-show="!scope.row.edit" @click="canEdit(scope)">{{ scope.row.showSort }}</span>
                    </template>
                  </sino-table-column>
                </sino-table>
    ...
    <!-- 应用到其它模版  configuration-application-box  size="small"-->
...
//        treeList: [],
    ...
    methods: {
      ...
      canEdit (data) {
        this.tableDataB[data.$index].edit = true;
        this.tableDataB = JSON.parse(JSON.stringify(this.tableDataB));
      },
      change (data) {
        let obj = {
        ...
        if (!checkedArr || checkedArr.length === 0) {
          ...
          api.post(this.apiPath.hrsc.chargeSave, obj1).then(res => {
            ...
            if (res.code === "CPYY-00001") {
              ...
              this.$route.query.chName = this.billMeterForm.chName;
              api.post(this.apiPath.hrsc.headFindByChName, {pageNum: 1, pageSize: 10, data: this.$route.query}).then(res => {
                this.tableDataA = res.data.list;
                this.total = res.data.total;
                this.checkLenth = 0;
              }).catch(err => {
