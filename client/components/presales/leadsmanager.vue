<template>
  ...
  <div class="leads usermanage">
      ...
      <sino-form :model="leadsForm" ref="ruleForm" class="serch-form leads-serch-form">
        <div class="search-box">
          <ul class="search-inner-box">
            ...
            <li>
              <span class="search-inner-name">线索跟进人：</span>
              <sino-select class="fl" :filterable="false" :multiselect="false" :isshowTotol="false" :disabled="false" :totalNum="false" :isPage="false" :isGroup="false" :chooseName.sync="leadsForm.trackName" :chooseId.sync="leadsForm.trackId" :linkage="true" :clear="true" :url="followSelUrl" :data="efData"></sino-select>
            </li>
            <li>
              <span class="search-inner-name">跟进结果：</span>
              <sino-select class="fl" :filterable="false" :multiselect="true" :isshowTotol="false" :disabled="false" :totalNum="false" :isPage="false" :isGroup="false" :chooseName.sync="leadsForm.trackResultName" :chooseId.sync="leadsForm.trackResults"  :clear="true" :url="selUrl" :data="{code:'TRACK_STATE', ids:[0, 1, 2, 3]}"></sino-select>
            </li>
            <sino-collapse-transition>
              <div class="sino-translate" v-show="searchFlag">
                <li>
                  <span class="search-inner-name">线索提供人：</span>
                  <sino-select class="fl" :filterable="true" :multiselect="false" :isshowTotol="false" :disabled="false" :totalNum="false" :isPage="true" :isGroup="false" :chooseName.sync="leadsForm.providerName" :chooseId.sync="leadsForm.providerId"  :clear="true" :url="custInfoUrl" :token="token"></sino-select>
                </li>
                <li>
    ...
    <div class="sino-table-total sino-table-radio mt15">
      <div class="sino-table-icon bs-none">
        <div class="sino-table-icon-left">
          ...
          <sino-button type="icon-text" @click="followFun"><i class="iconfont sino-follow"></i>分配跟进人</sino-button>
        </div>
      </div>
        ...
        <sino-table-column label="操作" align="center" width="180" fixed="right">
          <template scope="scope">
            <div v-if="scope.row.oper === true">
              <span class="table-span" @click="seeFun(scope.row)">查看</span><i class="table-i" v-if="scope.row.clueState === 1 || currUserRoleId == scope.row.trackId">|</i>
              <span class="table-span" @click="modifyFun(scope.row)" v-if="scope.row.clueState === 1 || currUserRoleId == scope.row.trackId">修改</span><i class="table-i" v-if="scope.row.clueState === 1 && currUserRoleId === scope.row.creId">|</i>
    ...
    <!--跟进-->
    <sino-dialog :visible.sync="follow" class="leads-follow leadsManagerBox" top="5%" v-on:close="followClose">
      ...
      <div class="follow-select">
        <sino-form :model="followForm" class="follow-form" :rules="followRules" ref="followForm">
          <div class="follow-content">
            ...
            <sino-form-item label="备注：" prop="trackRemark" class="sino-form-item-width">
              <sino-input type="textarea" v-model="followForm.trackRemark" :disabled="false"></sino-input>
            </sino-form-item>
          </div>
        </sino-form>
      </div>
      ...
      <div class="leadsAdd-form">
        <sino-form :model="followRemarkForm" :rules="followRemarkRules" ref="followRemarkForm" class="leadsAdd-Form">
          <sino-form-item label="跟进人：" prop="trackName">
            <sino-select :filterable="false" :multiselect="false" :isshowTotol="false"
                         :disabled="false" :totalNum="false" :isPage="false" :isGroup="false"
                         :chooseName.sync="followRemarkForm.trackName" :chooseId.sync="followRemarkForm.trackId"
          ...
          <sino-form-item label="跟进日期：" prop="operTime">
            <sino-date-picker
              ...
              :editable="false"
              :readonly="false"
              :clearable="true" @change="getSTime"
              :picker-options="pickerOptions"
            >
            </sino-date-picker>
          </sino-form-item>
          <sino-form-item label="跟进结果：" prop="trackResultName" class="clear">
            <sino-select :filterable="false" :multiselect="false" :isshowTotol="false" :totalNum="false" :isPage="false" :isGroup="false" :disabled="disabledFlag"
                         :chooseName.sync="followRemarkForm.trackResultName" :chooseId.sync="followRemarkForm.trackResult"
        ...
        <div class="follow-remark fl" v-if="!isA">
          <sino-form v-for="(list, index) in remarkHisList" :key="index">
            <sino-form-item label="跟进人：" prop="trackResultName">
              <sino-select :filterable="false" :multiselect="false" :isshowTotol="false"
  ...
  export default {
    ...
    data () {
      return {
        ...
        roleName: Cookie.get('CPYY-INFO-NAME'),
        roleId: Cookie.get('userRoleId'),
        ...
        followRules: {
          trackName: [
            {required: false, message: '请选择跟进人', trigger: 'blur'}
          ]
        },
...
//          this.ProviderName = res.data.userName;
          ...
          // console.log(this.currUserRoleId);
          // console.log(res.data);
          setTimeout(()  => {
            if (this.roleType == 4) {
              this.leadsForm.trackName = this.roleName;
              this.leadsForm.trackId = this.roleId;
            }
            // console.log(this.leadsForm.trackName);
            // console.log(this.leadsForm.trackId);
            this.pageNum = 1;
...
//        this.leadsAddForm.provider = this.ProviderName;
      ...
      followFun: function () {
        if (this.checkedArr.length !== 1) {
          ...
          let that = this;
          setTimeout(function () {
            that.fllowData.roleTypes = that.checkedArr[0].roleTypes;
          });
        }
      },
      // 查看
      ...
      followReSub () {
        ...
        _this.$refs.followRemarkForm.validate((valid) => {
          if (valid) {
            _this.followRemarkForm.operTime = Vue.filter('date')(_this.followRemarkForm.operTime, 'yyyy-MM-dd');
            ...
            api.post(_this.apiPath.crm.updateTrackRemarkById, _data).then(res => {
      ...
      followSure () {
        ...
        this.$refs.followForm.validate((valid) => {
          if (valid) {
            let _data = _this.followForm;
            _data.saleClueId = this.checkedArr[0].saleClueId;
            api.post(_this.apiPath.crm.updateSaleClueTrackId, _data).then(res => {
              if (res.code === 'CPYY-00001') {
                _this.follow = false;
                _this.leadsAdd = false;
                _this.searchButFun();
                _this.$message({message: '跟进人分配成功！', type: 'success'});
              }
            }).catch(err => {
              console.log(err);
            });
